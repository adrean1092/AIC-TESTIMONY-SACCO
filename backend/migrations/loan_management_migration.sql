-- 📄 loan_management_migration_fixed.sql
-- FULL LOAN MANAGEMENT MIGRATION (CORRECTED)
-- PostgreSQL-ready, safe to run multiple times

-- 1️⃣ Add loan_limit column to users if it doesn't exist
ALTER TABLE public.users
ADD COLUMN IF NOT EXISTS loan_limit NUMERIC(10,2) DEFAULT 0;

-- 2️⃣ Add initial_amount column to loans table if it doesn't exist
ALTER TABLE public.loans
ADD COLUMN IF NOT EXISTS initial_amount NUMERIC(10,2);

-- 3️⃣ Set initial_amount = amount for existing loans where it's NULL
UPDATE public.loans
SET initial_amount = amount
WHERE initial_amount IS NULL;

-- 4️⃣ Populate loan_limit for existing members (3 × total savings)
...
-- 5️⃣ Adjust loan_limit for members with active loans
UPDATE public.users u
...
-- 6️⃣ Function to update loan_limit when new savings are added
...
-- 7️⃣ Drop old savings trigger if exists and create new one
...
-- 8️⃣ Function to set initial_amount when a new loan is created
...
-- 9️⃣ Drop old loan trigger if exists and create new one
